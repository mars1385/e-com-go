...
func (lq *LoginQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Login, init func(*Login), assign func(*Login, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Login)
	for i := range nodes {
		if nodes[i].user_logins == nil {
			continue
		}
		fk := *nodes[i].user_logins
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_logins" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
